...
from taskonomy_loader import TaskonomyDatasetNew
...
def main(args):
    ...
    if 'nyu_v2' in args.data_dir:
        ...
        if not (args.data_dir.endswith('tiny') or args.data_dir.endswith('medium')):
            train_idx, val_idx = pickle.load(open('splits/split.pkl', 'rb'))
        else:
            train_idx, val_idx = None, None
        # train_idx, val_idx = pickle.load(open('splits/split.pkl', 'rb'))
        train_dataset = TaskonomyDatasetNew(
            args.data_dir,
            idx=train_idx,
            label_set=taskonomy_tasks,
            model_whitelist=train_model_whitelist,
    ...
    val_loader = get_eval_loader(args.data_dir, taskonomy_tasks, args, model_whitelist=val_model_whitelist, idx=val_idx)
    ...
    trainer.val_loader = get_eval_loader(args.data_dir, taskonomy_tasks, args, model_whitelist=test_model_whitelist, idx=val_idx)
...
def get_eval_loader(datadir, label_set, args, model_whitelist=None, idx=None):
    """
    ...
    if 'nyu_v2' in datadir:
        ...
        val_dataset = TaskonomyDatasetNew(datadir,
                                          idx=idx,
                                          label_set=label_set,
