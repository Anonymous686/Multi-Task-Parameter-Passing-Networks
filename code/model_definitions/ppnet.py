...
import torch
from .xception_taskonomy_sep import XceptionTaskonomySep, XceptionTaskonomySepSimple
from .resnet_taskonomy_sep import ResNetSep, ResNetSepSimple

__all__ = ['ppnet_distributed', 'backbone_distributed', 'ppnet_distributed_multicase', 'backbone_distributed_multicase',
           'backbone_distributed_simple', 'ppnet_distributed_simple']


class W_network_Copy(nn.Module):
    def __init__(self, model_num, non_linearity=False, layer_num=1, channel_wise=0, layer_shape=None):
        super().__init__()
        cur_shape = tuple(list(layer_shape) + [model_num])
        self.W_base = nn.Parameter(torch.ones(size=cur_shape, dtype=torch.float), requires_grad=True)

    def forward(self, param_x):
        assert self.W_base.shape == param_x.shape
        return self.W_base * param_x


class W_network_Residual(nn.Module):
    def __init__(self, model_num, non_linearity=False, layer_num=1, channel_wise=0, layer_shape=None, residual=False):
        super().__init__()
            ...
            if self.residual:
                ...
                self.W_base = nn.ParameterList(
                    [nn.Parameter(torch.eye(self.model_num, dtype=torch.float), requires_grad=True)
                     ...
                     range(layer_num)])
                # self.W_base = nn.ParameterList(
                #     [nn.Parameter(torch.ones((self.model_num, self.model_num), dtype=torch.float) / self.model_num, requires_grad=False)
                #      for _
                #      in
                #      range(layer_num)])
        else:
                    ...
                    if self.residual:
                        ...
                        W_base.append(
                            nn.Parameter(torch.eye(self.model_num * self.channels, dtype=torch.float),
                                         requires_grad=True))
                        # W_base.append(
                        #     nn.Parameter(torch.ones((self.model_num * self.channels, self.model_num * self.channels), dtype=torch.float) / (self.model_num * self.channels),
                        #                  requires_grad=False))
                elif i < layer_num - 1:
                    if self.residual:
                        W_base.append(
...
class PPNetDistributed(torch.nn.Module):
    def __init__(self, tasks: list, size=1, devices=None, conv_only=False, layer_wise=False, channel_wise=0,
        ...
        super(PPNetDistributed, self).__init__()
        ...
        if self.layer_wise:
            ...
            for name in self.names_to_aggregate:
                ...
                cur_W_network = W_network_Residual(model_num=self.model_num, non_linearity=False,
                                                   layer_num=1, channel_wise=self.channel_wise, layer_shape=param_shape,
                                                   residual=residual)
...
class BackboneDistributed(torch.nn.Module):
    def __init__(self, tasks: list, size=1, devices=None, backbone='xception'):
        super(BackboneDistributed, self).__init__()
        self.model_num = len(tasks)
        self.tasks = tasks
        self.backbone = backbone
...
class BackboneDistributedSimple(torch.nn.Module):
    def __init__(self, tasks: list, size=1, devices=None, backbone='xception', num_seg_cls=19):
        super(BackboneDistributedSimple, self).__init__()
        self.model_num = len(tasks)
        self.tasks = tasks
        self.backbone = backbone
        ...
        if self.backbone == 'xception':
            self.models = nn.ModuleList([XceptionTaskonomySepSimple(tasks[i], size=size, num_seg_cls=num_seg_cls)
                                         for i in range(self.model_num)])
        elif self.backbone == 'resnet':
            # currently, using resnet34
            self.models = nn.ModuleList([ResNetSepSimple(tasks[i], size=size, num_seg_cls=num_seg_cls)
                                         for i in range(self.model_num)])
        else:
            raise NotImplementedError('Unknown backbone:', self.backbone)
...
class PPNetDistributedMultiCase(torch.nn.Module):
    def __init__(self, tasks: list, size=1, devices=None, conv_only=False, layer_wise=False, channel_wise=0,
                 residual=False, backbone='xception'):
        super(PPNetDistributedMultiCase, self).__init__()
        self.model_num = len(tasks)
        self.tasks = tasks
        self.backbone = backbone
        self.conv_only, self.layer_wise = conv_only, layer_wise
...
class BackboneDistributedMultiCase(torch.nn.Module):
    def __init__(self, tasks: list, size=1, devices=None, backbone='xception'):
        super(BackboneDistributedMultiCase, self).__init__()
        self.model_num = len(tasks)
        self.tasks = tasks
        self.backbone = backbone
...
class PPNetDistributedSimple(torch.nn.Module):
    def __init__(self, tasks: list, size=1, devices=None, conv_only=False, layer_wise=False, channel_wise=0,
                 residual=False, backbone='xception', num_seg_cls=19):
        super(PPNetDistributedSimple, self).__init__()
        self.model_num = len(tasks)
        self.tasks = tasks
        self.backbone = backbone
        self.conv_only, self.layer_wise = conv_only, layer_wise
        ...
        if self.backbone == 'xception':
            self.models = nn.ModuleList([XceptionTaskonomySepSimple(tasks[i], size=size, num_seg_cls=num_seg_cls)
                                         for i in range(self.model_num)])
            self.fake_models = nn.ModuleList([XceptionTaskonomySepSimple(tasks[i], size=size, num_seg_cls=num_seg_cls)
                                              for i in range(self.model_num)])
        elif self.backbone == 'resnet':
            # currently, using resnet34
            self.models = nn.ModuleList([ResNetSepSimple(tasks[i], size=size, num_seg_cls=num_seg_cls)
                                         for i in range(self.model_num)])
            self.fake_models = nn.ModuleList([ResNetSepSimple(tasks[i], size=size, num_seg_cls=num_seg_cls)
                                              for i in range(self.model_num)])
        else:
            raise NotImplementedError('Unknown backbone:', self.backbone)
...
def ppnet_distributed(tasks: list, size=1, devices=None, **kwargs):
    return PPNetDistributed(tasks=tasks, size=size, devices=devices, **kwargs)


def ppnet_distributed_multicase(tasks: list, size=1, devices=None, **kwargs):
    return PPNetDistributedMultiCase(tasks=tasks, size=size, devices=devices, **kwargs)


def backbone_distributed(tasks: list, size=1, devices=None, **kwargs):
    return BackboneDistributed(tasks=tasks, size=size, devices=devices, **kwargs)


def backbone_distributed_multicase(tasks: list, size=1, devices=None, **kwargs):
    return BackboneDistributedMultiCase(tasks=tasks, size=size, devices=devices, **kwargs)


def ppnet_distributed_simple(tasks: list, size=1, devices=None, **kwargs):
    return PPNetDistributedSimple(tasks=tasks, size=size, devices=devices, **kwargs)


def backbone_distributed_simple(tasks: list, size=1, devices=None, **kwargs):
    return BackboneDistributedSimple(tasks=tasks, size=size, devices=devices, **kwargs)