...
class MTANDeepLabv3(nn.Module):
    def __init__(self, tasks):
        ...
        self.tasks = tasks
        # backbone = ResnetDilated(resnet18(pretrained=False))
        backbone = ResnetDilated(resnet34(pretrained=False))
        # for resnet50
    ...
    def forward(self, x):
        ...
        for i in range(len(self.tasks)):
            ret = F.interpolate(self.decoders[i](a_4[i]), size=256, mode='bilinear', align_corners=True)
